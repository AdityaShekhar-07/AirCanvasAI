...
# 1. Higher confidence for stable tracking on your G14
...
# 2. Resizable Window Setup
win_name = "VisionWrite AI"
cv2.namedWindow(win_name, cv2.WINDOW_NORMAL) # Allows manual resizing
...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            # Always draw skeleton for feedback
            ...
            if len(mode_buffer) > BUFFER_SIZE: mode_buffer.pop(0)
    ...
    cv2.putText(frame, f"MODE: {current_mode}", (w-250, 45), 
